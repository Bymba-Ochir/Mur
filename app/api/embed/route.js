...
// зөвхөн серверийн орчинд (.env.local, NEXT_PUBLIC_ угтваргүй) хадгална,
...
export async function POST(request) {
  ...
  if (!token) {
    return NextResponse.json(
      { error: 'HUGGINGFACE_API_TOKEN тохируулагдаагүй байна. .env.local шалгана уу.' },
      { status: 500 }
    );
  }
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: `Hugging Face алдаа: ${errText}` },
        { status: 502 }
      );
    }
